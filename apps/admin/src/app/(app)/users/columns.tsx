...
function FullNameCell({ user }: { user: UserRow }) {
  const { openUser } = usePersonModal()
  return (
    <button
      type="button"
...
function ViewUserButtonCell({ userId }: { userId: string }) {
  const { openUser } = usePersonModal()
  return (
  ...
  return [
    {
      ...
      cell: ({ row }) => <FullNameCell user={row.original} />,
    },
    {
      id: 'roles',
    ...
    },
    {
      ...
      cell: ({ row }) => <ViewUserButtonCell userId={row.original.id} />,
    },
  ]
}
