...
**Reference (оновлено 2026-08-07):** структурний kit лишається shadcn/ui —
Radix-примітиви в `components/ui/*` (ADR 0002), включно з `Sidebar`/
`SidebarProvider` (не переписувати вручну). `reference-admin/`
(`satnaing/shadcn-admin`) як джерело копіювання компонентів більше не
використовується — чекаут так і не було зроблено, а власна візуальна система
замінила його: навy сайдбар/навігація + світла робоча область + золотий
акцент (nod до «Starland»), тон — tgi-platform.com (професійний, мінімалістичний,
контрастний navy/white), не копіюється буквально (конкурентна платформа для
шкіл). Токени — `apps/admin/src/app/globals.css`. Дашборд і надалі не показує
вигаданих SaaS-метрик (Revenue/Sales) — лише шкільний зміст.
